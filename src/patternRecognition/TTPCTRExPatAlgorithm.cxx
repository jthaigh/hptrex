...
void ND::TTPCTRExPatAlgorithm::Process(const std::vector<ND::TTPCHitPad>& hits, std::vector<ND::TTPCHitPad*>& used, std::vector<ND::TTPCHitPad*>& unused){
  ...
  for(unsigned int i=0; i<subEvents.size(); ++i){
    ...
    if(fMasterVolGroupMan->CheckUsability(subEvent)){
      ...
      fSubAlgorithms.push_back(ND::TTPCTRExPatSubAlgorithm(fMasterLayout));
